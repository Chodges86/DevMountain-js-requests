...
sayHelloBtn.addEventListener("mouseover", hoverBlackTheme)

function hoverBlackTheme(event) {
    event.target.classList.add("black-and-white")
}
...
sayHelloBtn.addEventListener("mouseout", removeBlackTheme)

function removeBlackTheme(event) {
    event.target.classList.remove("black-and-white")
}
...
const repeatMyParam = () => {
  //YOUR CODE HERE
  const param = "CUSTOM PARAM"
  axios
    .get(`http://localhost:3000/repeat/${param}`)
    .then(res => {
        console.log(res.data)
        const repeatMyParamElement = document.querySelector("#repeat-text")
        repeatMyParamElement.textContent = param
        repeatMyParamElement.style.display = "block"
    })
};
...
function sendQuery() {
    ...
    axios   
        .get(`http://localhost:3000/query-test/?${query}`)
        .then(res => {
            console.log(res.data)
            const repeatMyParamElement = document.querySelector("#repeat-text")
        repeatMyParamElement.textContent = query
        repeatMyParamElement.style.display = "block"
        })
}

const sendQueryBtn = document.querySelector("#query-button")
...
/*
    ...
    console logged "You sent an empty query!"

    2: Send more than 1 query on the URL -- what happened? 

    console logged "You sent query: {1st query},{2nd query} with value: !"
*/
...
function createFood(event) {
    ...
    .then(res=> {
        res.data.forEach(value => {
            ...
            pElement.textContent = value
            foodContainer.appendChild(pElement)
        })
    })
    .catch(err => console.log(err))

}
let addFoodForm = document.querySelector("form")
